...
function handle_message(e) {
  ...
  if (e.data.type === "requestFrame") {
    ...
    const outputPanels = [];
    const transferrableBuffers = [];
    const panel_start_time = performance.now();
    for (const panel of e.data.panels) {
      if (panel.id === "screen") {
        const image_buffer = get_screen_pixels(panel.dest_buffer);
        outputPanels.push({
          id: "screen",
      ...
      if (panel.id === "piano_roll_window") {
        const image_buffer = get_piano_roll_pixels(panel.dest_buffer);
        outputPanels.push({
    ...
    const audio_buffer = consume_audio_samples();
